...
public interface IOrdersRepository extends CrudRepository<OrderCM, Integer> {
    @Query("SELECT o FROM Orders o WHERE o.dealer.dealerNumber = :dealerNumber ")
    List<OrderCM> findByDealerNumber(@Param("dealerNumber") String dealerNumber);
    @Query("SELECT o FROM Orders o WHERE o.dealer.dealerNumber = :dealerNumber AND o.deliveryStatus.code = :deliveryStatus")
